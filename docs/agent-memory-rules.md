...
- **進捗は Linear だけに書く**（2026-09-23〜、JUK-33）。残り・作業中・完了・次の一手を
  メモリとの二重管理にしない。Project・Cycle・Status Update は使わない（1人開発で管理が
  作業にならないように）。
- **作業開始時**は、Linear の In Progress / Todo でタスクの全体像を、In Progress の
  最新コメントで前回の到達点と次の一手を、auto-memory（`MEMORY.md` と関連トピック
...
3. 後のセッションで必要になる恒久的な文脈だけを記録する：決定、検証結果・実測値、
   ブロッカーの理由、警告。これからやるタスクと次の一手は Linear に書き、メモリには
   Issue 番号（`JUK-xx`）と文脈だけを書く（「タスク管理」セクション参照）。
4. 古くなった TODO や、リポジトリの実態と矛盾する記述は、削除または書き換える。
5. `MEMORY.md` の索引は簡潔に保ち、「何のトピックか」だけを書く。「完了」「次は〜」
   のような状態は書かない。
