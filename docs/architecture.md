...
ここから先は「どう作ったか」ではなく「**なぜそう決めたか**」を書く。実装は git 履歴を
見れば分かるが、選ばなかった選択肢とその理由はコードに残らないため。

### データ取得の入口を REST API に統一し、Server Actions は使っていない

このリポジトリに Server Actions は**1つも無い**（`"use server"` は 0 件）。以前は
ログアウト処理だけが Server Action だったが、Better Auth のクライアントから既存の
認証エンドポイントを呼ぶ方式へ変更して削除した。

**理由: 将来スマートフォンアプリから同じサーバーを使いたいから。** Server Actions は
ブラウザから Next.js を呼ぶ専用の仕組みで、外部クライアントからは実質叩けない。

- **URL が固定されない。** Server Action にはエンドポイントの URL が無い。呼び出しは
  ページ自身への POST として送られるので、「この機能はこの URL」という約束を外部に
  公開できない。
- **識別子がビルドごとに変わる。** どの関数を実行するかは `Next-Action` ヘッダーに載る
  ID で決まる。この ID はビルド時に生成されるハッシュなので、デプロイのたびに変わる。
  アプリ側に焼き込むと次のデプロイで壊れる。
- **ボディが React 独自形式。** 引数は普通の JSON ではなく React Server Components の
  シリアライズ形式で送られる。仕様は公開 API ではなく、Swift や Kotlin から組み立てる
  現実的な手段が無い。

REST なら `POST /api/study-logs` に JSON を送るだけで、Web もアプリも同じ入口を使える。
Server Component からの初回表示だけはサービス層を直接呼び（自分自身への HTTP を
経由しないので速い）、ブラウザからの更新は REST を通す、という二経路になっている。
この二経路が同じ形を返すことは `src/shared/dto/` の型と `src/backend/dto/study-mapper.ts`
が保証している。

### UI 層から Prisma を import しない

**理由は「DB を隠すため」ではなく「同じクエリが増殖するのを防ぐため」。** 実際に
`app/profile/page.tsx` と `app/api/notification-preferences/route.ts` に1文字違わず同じ
`findUnique` が書かれていた（`docs/performance.md` の TASK 3）。片方だけ `select` を
直せば、画面と API で返るデータが静かにずれる。

サービス層に集約すれば、直す場所は1つになる。加えて全サービスが `measured()` を通るので、
どのクエリが遅いかがログから追える。UI に直書きされたクエリはこの計測から漏れる。

このルールは ESLint で強制している（上記）。
...
リポジトリ層の主な利点は「DB を差し替えられること」だが、MySQL から移る予定は無い。
一方で失うものは具体的である。

- **Prisma の型推論が切れる。** `include` の内容に応じて戻り値の型が変わるのが Prisma の
  最大の利点で、`src/backend/dto/study-mapper.ts` はその型を
  `Prisma.StudyPlanGetPayload<{ include: ... }>` として受けている。schema を変えると
  変換側が型エラーになり、直し忘れに気づける。リポジトリで自前の型に詰め替えると、
  この検出が効かなくなる。
- **`select` の柔軟性が失われる。** 画面ごとに必要な列は違う。`listGoalsWithFaculty` は
  `tags` を引かず `listGoals` は引く、という使い分けをしている（`tags` は暗黙的多対多
  なので SQL が1本増える）。リポジトリのメソッドを固定すると、この差を表現するために
  メソッドが際限なく増える。
- **層が1つ増える。** 個人開発の規模では、通過するだけのファイルが増える負担が
  差し替え可能性という使わない利点を上回る。

**この判断を見直す条件**は、DB を実際に移す必要が出たとき、または Prisma を直接
モックできずテストが書けない場面が繰り返し出てきたとき。現状はサービス層の外側
（Route Handler のテスト）で `vi.mock("@/backend/infra/prisma")` によりモックできている。

### 学習用に別途フルスタック分離を進めている理由

Next.js はフロントとバックが同じプロセスに同居する設計なので、この構成での
frontend / backend の境界は**フォルダと ESLint による約束**であり、物理的な分離ではない。
`app/` が両方を import できるのは、両者が1つのプログラムにコンパイルされるためである。

本当にプロセスを分けた場合、`app/` に相当する「両方を呼ぶ入口」は存在しなくなり、
frontend と backend は HTTP でしか通信できなくなる。共有も import ではなく、
API 契約（OpenAPI 等）で合わせることになる。その理解を得るために、別リポジトリ
`juken-map-fullstack-lab`（React + Vite / Fastify / 将来 Go）を学習目的で進めている。
本番アプリを分離するかどうかは、そこで得た理解を踏まえて改めて判断する。
