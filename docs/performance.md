...
計測にもとづく構造改善の記録。数字は全て開発環境での実測値で、本番では測っていない。

> **このファイルは2つの時代をまたぐ。** 「TASK 1〜7」は Next.js + Prisma 時代（2026-09-09）の
> 記録で、当時の構成を前提にした記述をそのまま残している（`lib/prisma.ts`、
> `.next/dev/logs/`、`schema.prisma` などは**現在は存在しない**）。以降の節は
> SPA + Fastify + 生 SQL の構成で測ったもので、節ごとに計測環境を書いている。

## 計測の標準手順

### 現行（SPA + Fastify + 生 SQL）

SQL そのものを見たいときは、DB に直接 `EXPLAIN ANALYZE` を流す。実行計画（索引を使ったか、
ソートが入ったか）と、接続コストを含まないサーバ側の所要時間が同時に取れる。ばらつくので
複数回とって中央値を使う。

アプリが1リクエストで何本 SQL を流すかを見たいときは、`apps/api/src/infra/db.ts` の
クエリログ（Prisma の `log: ["query"]` の代わり）を使う。

### （旧）Next.js 時代の手順

本番環境ではなく開発サーバーで、SQL の発行本数を数える。Prisma のクエリログは
`lib/prisma.ts` で開発環境のみ有効化してあった。
...
計測はクライアント JS を実行しない curl で行った。ブラウザで開くと、ページ描画の後に
...
# 全タスクを通した改善サマリ（TASK 1〜7・2026-09-09）

## 数字で測れた改善

計測は開発サーバー・ローカル MySQL・e2e ユーザーでの `/dashboard` 1回 GET。
レスポンス時間は開発サーバーの値なので参考値で、意味があるのは SQL 本数のほう。

| 指標 | 着手前 | 完了後 | 差分 |
|---|---:|---:|---|
| `/dashboard` の `prisma:query` | 12 本 | **10 本** | **-17%** |
| うち認証関連の SQL | 4 本 | **2 本** | **-50%** |
| `getSession()` の実行回数 | 2 回 | **1 回** | -1 |
| レスポンス時間（参考） | 0.134s / 0.115s | 0.098s / 0.091s | 約 -25% |

**効果が最も大きかったのは TASK 1（`getSession` の重複解消）で、実質これだけが
数値改善である。** React の `cache()` で包んだだけの変更だが、Header は全ページ共通なので
**ログイン中の全ページ遷移で毎回2本減る**。範囲の広さが効いた。

TASK 3 以降は SQL 本数を1本も変えていない。構造の変更であって性能改善ではないため、
数字が動かないことが正しい結果だった。「変わっていないこと」を毎回計測して確認している。

### 意図的に見送った改善

TASK 2 で `/dashboard` の 10 本のうち **5 本が 0 件確定の空振り**と判明したが、
`relationLoadStrategy: "join"`（10本 → 6本、速度は約半分）は**採用しなかった**。
空振り 5 本のコストがローカル実測で約 3ms（ページ全体 約90ms の数%）に対し、
Prisma の preview 機能 `relationJoins` への依存を新たに負うため。検証結果は残してあるので、
実データで問題が出た時点で再検証なしに再検討できる。

**性能改善は、測ったうえでやらないと決めることも結果である。**

## 数字で測れなかった改善（構造）

こちらが今回の主目的だった。何が良くなったかを言語化しておく。

### 1. 同じことを2箇所に書く状態が消えた

これが最大の成果である。着手前は、片方だけ直せば静かにずれる箇所が実在していた。

- `app/profile/page.tsx` と `app/api/notification-preferences/route.ts` に、
  **1文字違わず同じ `findUnique`**（TASK 3）
- StudyPlan の DTO 変換が**4箇所**。うち API は変換を書いていないのに
  `JSON.stringify` の暗黙の Date 変換で形が一致していた＝**一致の根拠が偶然**（TASK 4）

いずれもフィールドを1つ足すだけで壊れる。今は直す場所が1つになっている。

### 2. 壊し方が「静かな不具合」から「ビルドエラー」に変わった

構造そのものより、この変化のほうが実務的な価値が大きい。

| 崩れ方 | 着手前 | 完了後 |
|---|---|---|
| UI から DB を直接触る | 気づかない | **ESLint エラー**（TASK 6） |
| frontend が backend を呼ぶ | 気づかない | **ESLint エラー**（TASK 6） |
| schema 変更で DTO がずれる | 実行時まで気づかない | **型エラー**（`Prisma.StudyPlanGetPayload` から導出・TASK 4） |
| クエリを画面に直書き | 計測から漏れる | サービス層は全て `measured()` を通る |

### 3. どちらのコードかがディレクトリで分かる

`lib/` に41本が平置きされ、開かないとフロント用かバック用か分からなかった状態から、
`src/frontend`（65本）/ `src/backend`（19本）/ `src/shared`（12本）になった。
分類は責務で行い、参照元を機械的に数えて決めている（`docs/architecture.md`）。

### 4. 判断の理由が残った

`docs/architecture.md` に、**選ばなかった選択肢とその理由**を書いた。リポジトリ層を
入れない理由、Server Actions を使わない理由、`subjects.ts` を分割しない理由など。
実装は git 履歴を見れば分かるが、これはコードに残らない。

## 検証方法として確立したもの

構造変更で「何も変わっていないこと」を目視ではなく機械的に示す手順が固まった。
詳細は `docs/architecture.md` の「検証方法」にある。要点は3つ。

1. **`git stash` で同じ dev server から変更前後を取得して比較する。** ビルドし直さないので
   環境差が入らない
2. **HTML は表示 DOM だけを比較する。** RSC ペイロードにはリクエスト毎のランダム ID と
   flight payload の内部参照番号のずれが必ず出るため、そのままでは比較にならない
3. **生成 CSS も比較する。** Tailwind v4 の class 検出崩れは HTML 差分では**絶対に
   検出できない**（class 名は HTML に残り、CSS だけが欠ける）。ディレクトリ移動では必須

データが0件だと描画経路を通らないので、e2e ユーザー（学習記録あり・志望校0件）と
demo ユーザー（志望校4件）の両方で確認する。

## 残っている課題（TASK 1〜7 時点・2026-09-09）

**Route Handler にロジックが多く残っている。** TASK 3 は UI 層（`.tsx`）だけを対象に
したため、`app/api/` の24ファイル中16ファイルが Prisma を直接呼んでいる
（読み取り37件・書き込み53箇所ぶん）。書き込みは Zod 検証・所有者チェック・
`demoReadOnlyGuard`・`P2002 → 409` の変換が絡むので、サービス層へ移すなら設計判断が要る。

> **決着済み（2026-09-10〜09-11）。** SPA 分離で routes / services に整理され、
> ORM も外れた。`apps/api/src/routes/` から DB を直接触る箇所は 0 件である
> （`docs/architecture.md`「routes は DB を触らない」）。

---

# DB 索引の計測（2026-09-12）

ORM を外して SQL が見えるようになったので、索引が実際に使われているかを EXPLAIN で確かめた。

## 計測環境

| 項目 | 値 |
|---|---|
| MySQL | 8.0.46（**本番と CI は 8.4**。実行計画の傾向は同じだが、数値はこの環境のもの） |
| 計測方法 | `EXPLAIN ANALYZE` の最上段 `actual time` の中央値（15回）。接続コストを含まないサーバ側の時間 |
| データ | 主役ユーザー 1人に実績 5,500件（3年分）＋ 他199人に各1,500件＝ StudyLog 304,000行、StudyPlan 219,008行 |

**なぜこの規模で測るか。** 本番の利用者は開発者本人のみなので、今日の本番に体感できる遅さは
無い。測りたいのは「記録が貯まったとき」と「利用者が増えたとき」に何が最初に壊れるかである。

## 事実: 日次通知（cron）が全表走査になっていた

日次通知は対象ユーザーをまとめて `WHERE userId IN (...) AND date >= ? AND date < ?` で引く。
`userId` だけの索引は、IN に全ユーザーが並ぶと選択的でなくなるため、**オプティマイザが
索引を捨てて全表走査を選ぶ。**

| クエリ | 索引追加前 | 追加後 | 倍率 |
|---|---|---|---|
| cron の実績取得（200ユーザー・当日分） | **292 ms**（StudyLog 304,000行を走査して404行を返す） | **2.7 ms** | 約110倍 |
| cron の予定取得（200ユーザー・当日分） | **310 ms**（StudyPlan 219,008行を走査して200行を返す） | **1.9 ms** | 約160倍 |

**利用者が1人でも効く。** 1ユーザーに絞った同じクエリでも、`userId` だけの索引では
そのユーザーの5,500行を全部読んでから日付で絞っていた（8.7 ms）。`(userId, date)` なら
範囲走査で6行だけ読む（0.05 ms）。**遅さは利用者数ではなく、1人あたりの記録量で決まる。**

## 事実: 一覧の並び順によって、同じ索引が効いたり効かなかったりする

InnoDB の二次索引には主キーが暗黙に付くので、`(userId, date)` は実質 `(userId, date, id)`
として働く。したがって**索引と同じ向きに並べるときだけ**、ソートを省略できる。

| 一覧 | ORDER BY | 結果 |
|---|---|---|
| `GET /api/study-plans` | `date ASC, id ASC` | **ソートが消えた**（索引の並びと一致） |
| `GET /api/study-logs` | `date DESC, id ASC` | **ソートが残る**（`date` は逆順、`id` は正順で向きが混ざる） |

実績一覧の `id ASC` は、同じ日付の中の順番を決めるために足したものである
（`apps/api/src/services/study-log-service.ts`）。この1語が索引の逆順走査を使えなくしている。

### 並び順を揃えると何が変わるか（未実施・要判断）

`date DESC, id DESC` に変えると、索引の逆順走査でソートが消える。**ただし全件を返している
限り、体感差はほとんど無い。**

| 条件 | `date DESC, id ASC`（現行） | `date DESC, id DESC` |
|---|---|---|
| 全件（5,500件） | 9.4 ms | 8.7 ms |
| **直近50件だけ（LIMIT 50）** | **7.8 ms** | **0.1 ms** |

LIMIT が付いて初めて約78倍になる。**「並び順を揃える」と「件数を絞る」はセットでないと
意味が無い。** 同じ日付の中の表示順が変わるため、画面の仕様変更を伴う。

## 確かめて外れた仮説

書き残しておく。どちらも「直すべきもの」に見えて、実際には問題ではなかった。

1. **`userId(191)` のプレフィックス索引。** マイグレーションの SQL には
   `INDEX StudyLog_userId_idx(userId(191))` と書かれているが、`SHOW CREATE TABLE` では
   `KEY StudyLog_userId_idx (userId)` になる。**プレフィックス長が列長と同じなので、MySQL が
   通常の索引に丸めている。** 覆う索引にならない・ORDER BY に使えないといった不利は無い。
2. **`GET /api/universities` が823大学を毎回全件返している。** 実測は903行・4.0 ms で、
   DB 側の問題ではなかった（マスターに学部があるのは66件のみ）。絞り込みを画面側で
   やっていること自体は残るが、**索引や SQL で直す対象ではない。**

## 実施したこと

`db/migrations/20260912000000_add_user_date_indexes/migration.sql`

- `StudyPlan(userId, date)` と `StudyLog(userId, date)` を追加
- `StudyPlan(userId)` と `StudyLog(userId)` を削除。複合索引の先頭列と重なり、
  **外部キーが要求する索引も複合索引が満たす**ことを実機で確認した
  （先に複合索引を作ってから単独索引を落とせる）
- 既存データ入りの DB（StudyLog 304,000行・StudyPlan 219,008行）への適用は 2.5 秒。
  本番のデータ量ははるかに小さいので、長いロックにはならない

## 次にやるなら

- **一覧 API に件数の上限を入れる**（`GET /api/study-logs` / `/api/study-plans` は現在
  全件を返す）。上の表のとおり、効くのは LIMIT を入れてからである。並び順の変更と
  API 契約・画面の変更を伴うので、設計判断が要る
- 期間での絞り込み（カレンダーは月単位で表示するので、月の範囲で引ければ十分なはず）
