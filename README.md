...
USERS=2000 MONTHS=12 pnpm run db:seed:synthetic  # 2000人 × 12か月分（約17万件の学習実績）
```

行数を増やすだけでなく、**使い方の偏り**を作ります。続ける人・だんだん減って止まる人・
たまに使う人・登録しただけの人を混ぜ、平日と休日の差、科目の得意不得意、
予定を立てたのに手を付けなかった日も作ります。全員が毎日きちんと記録する前提のデータだと、
索引もキャッシュも実際より効いてしまうためです。2000人の場合、直近7日に記録がある人は
全体の約25%になります。

同じ`SEED`なら何度実行しても同じデータになり、実行のたびに入れ直します。
合成した利用者は`@synthetic.juken-map.invalid`のメールアドレスを持ち、全員同じパスワードで
ログインできるので、k6の負荷試験からそのまま使えます。手元のDB以外へは流れません
